...
function handleDeleteEvents() {
  let __products = JSON.parse(localStorage.getItem("products"));
  ...
  Array.from(buttonsDelete).forEach((buttonDelete) => {
    buttonDelete.addEventListener("click", () => {
      ...
      yesButton.addEventListener("click", () => {
        __products = __products.filter((element) => {
          return element.id !== buttonDelete.value;
        });
        localStorage.setItem("products", JSON.stringify(__products));
...
function renderProducts() {
  ...
  let paginatedProducts = __products.slice(start, end);
  ...
  renderPagination(__products.length, itemsPerPage);
}
